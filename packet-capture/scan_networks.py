...
def parse_airodump(interface):
    try:
        # Use airodump-ng with provided interface
        # --manufacturer: show manufacturer
        # -w /tmp/dump: output prefix (we'll parse CSV)
        # --background 1: update every 1 second
...
def main():
    ...
    # If no results, try iw scan as fallback
    if not networks:
